...
class Animation:
    ...
    def draw(self, surface, position):
        """
        현재 프레임을 화면에 그리기
        :param surface: pygame.Surface (그릴 화면)
        :param position: (x, y) 튜플 또는 벡터 (그릴 위치)
        :param flip: 수평 반전 여부
        """
        ...
        surface.blit(frame_image, position)
...
# 플레이어 설정
player_pos = pygame.Vector2(640, 320)
player_vel = pygame.Vector2(0, 0)
player_grounded = False
player_flip = False

# 애니메이션 생성
run_animation = Animation("anim.png", num_frames=10, frame_length=0.1, loop=True)
...
running = True
while running:
    dt = clock.tick(30) / 1000  # 초 단위의 프레임 시간
    ...
    # 바닥 충돌 처리
    if player_pos.y > SCREEN_HEIGHT - 64:
    ...
    # 애니메이션 프레임 그리기
    current_animation.draw(screen, player_pos, flip=player_flip)

    # 현재 시간 텍스트 생성 (정수 부분 항상 0으로 표시)
    current_time = round(time.time() % 1, 1)  # 초의 소수점 이하만 계산
    time_text = font.render(f"0.{str(current_time)[2:]}", True, (255, 255, 255))  # 항상 0으로 시작
    screen.blit(time_text, (10, 10))  # 좌측 상단 (10, 10)에 출력
